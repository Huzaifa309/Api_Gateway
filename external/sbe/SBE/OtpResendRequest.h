...
#ifndef _SBE_OTPRESENDREQUEST_CXX_H_
#define _SBE_OTPRESENDREQUEST_CXX_H_
...
public:
    ...
    OtpResendRequest(char *buffer, const std::uint64_t bufferLength) :
        OtpResendRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OtpResendRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OtpResendRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OtpResendRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OtpResendRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    OtpResendRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        OtpResendRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OtpResendRequest &_writer)
{
    OtpResendRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "OtpResendRequest", )";
