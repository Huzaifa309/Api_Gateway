...
void eloan::VCMModule::onMarketData(uint32_t instrument_id, double best_bid, double best_ask) {
    // Simplified: do nothing
}

bool eloan::VCMModule::checkSpread(const Order &order) {
...
bool eloan::VCMModule::checkVolatility(const Order &order) {
