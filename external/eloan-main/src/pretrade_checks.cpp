...
bool eloan::PreTradeChecks::checkMaxOrderQty(const Order &order) {
...
bool eloan::PreTradeChecks::checkPriceBand(const Order &order, double reference_price) {
...
bool eloan::PreTradeChecks::checkPositionLimit(const Order &order) {
    int shard = order.account_id % NUM_SHARDS;
