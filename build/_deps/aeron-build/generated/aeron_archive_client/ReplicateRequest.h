...
#ifndef _AERON_ARCHIVE_CLIENT_REPLICATEREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_REPLICATEREQUEST_CXX_H_
...
public:
    ...
    ReplicateRequest(char *buffer, const std::uint64_t bufferLength) :
        ReplicateRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ReplicateRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ReplicateRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ReplicateRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ReplicateRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ReplicateRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        ReplicateRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ReplicateRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ReplicateRequest &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ReplicateRequest &srcRecordingId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ReplicateRequest &dstRecordingId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ReplicateRequest &srcControlStreamId(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    ReplicateRequest &putSrcControlChannel(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ReplicateRequest &putSrcControlChannel(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    ReplicateRequest &putSrcControlChannel(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    ReplicateRequest &putLiveDestination(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    ReplicateRequest &putLiveDestination(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    ReplicateRequest &putLiveDestination(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ReplicateRequest &_writer)
{
    ReplicateRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ReplicateRequest", )";
