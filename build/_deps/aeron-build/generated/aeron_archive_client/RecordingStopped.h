...
#ifndef _AERON_ARCHIVE_CLIENT_RECORDINGSTOPPED_CXX_H_
#define _AERON_ARCHIVE_CLIENT_RECORDINGSTOPPED_CXX_H_
...
public:
    ...
    RecordingStopped(char *buffer, const std::uint64_t bufferLength) :
        RecordingStopped(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    RecordingStopped(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        RecordingStopped(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    RecordingStopped &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    RecordingStopped &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    RecordingStopped &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        RecordingStopped skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    RecordingStopped &recordingId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    RecordingStopped &startPosition(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    RecordingStopped &stopPosition(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const RecordingStopped &_writer)
{
    RecordingStopped writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "RecordingStopped", )";
