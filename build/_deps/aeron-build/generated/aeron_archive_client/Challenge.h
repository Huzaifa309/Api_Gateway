...
#ifndef _AERON_ARCHIVE_CLIENT_CHALLENGE_CXX_H_
#define _AERON_ARCHIVE_CLIENT_CHALLENGE_CXX_H_
...
public:
    ...
    Challenge(char *buffer, const std::uint64_t bufferLength) :
        Challenge(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Challenge(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Challenge(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Challenge &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Challenge &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Challenge skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Challenge &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    Challenge &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    Challenge &version(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *encodedChallengeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static const char *encodedChallengeCharacterEncoding() SBE_NOEXCEPT
    {
        return "null";
    }

    static SBE_CONSTEXPR std::uint64_t encodedChallengeSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool encodedChallengeInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    static SBE_CONSTEXPR std::uint16_t encodedChallengeId() SBE_NOEXCEPT
    {
        return 4;
    }

    static SBE_CONSTEXPR std::uint64_t encodedChallengeHeaderLength() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD std::uint32_t encodedChallengeLength() const
    {
        std::uint32_t length;
    ...
    std::uint64_t skipEncodedChallenge()
    {
    ...
    SBE_NODISCARD const char *encodedChallenge()
    {
        std::uint32_t lengthFieldValue;
    ...
    std::uint64_t getEncodedChallenge(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    Challenge &putEncodedChallenge(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    std::string getEncodedChallengeAsString()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    std::string getEncodedChallengeAsJsonEscapedString()
    {
        std::ostringstream oss;
        std::string s = getEncodedChallengeAsString();
    ...
    #if __cplusplus >= 201703L
    std::string_view getEncodedChallengeAsStringView()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    Challenge &putEncodedChallenge(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putEncodedChallenge(str.data(), static_cast<std::uint32_t>(str.length()));
    }

    #if __cplusplus >= 201703L
    Challenge &putEncodedChallenge(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Challenge &_writer)
{
    Challenge writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("encodedChallenge": )";
    builder << '"' <<
        writer.skipEncodedChallenge() << " bytes of raw data\"";
    builder << '}';
...
SBE_NODISCARD static std::size_t computeLength(std::size_t encodedChallengeLength = 0)
{
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += encodedChallengeHeaderLength();
    if (encodedChallengeLength > 1073741824LL)
    {
        throw std::runtime_error("encodedChallengeLength too long for length type [E109]");
    }
    length += encodedChallengeLength;
