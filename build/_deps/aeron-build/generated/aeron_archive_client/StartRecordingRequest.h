...
#ifndef _AERON_ARCHIVE_CLIENT_STARTRECORDINGREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_STARTRECORDINGREQUEST_CXX_H_
...
public:
    ...
    StartRecordingRequest(char *buffer, const std::uint64_t bufferLength) :
        StartRecordingRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    StartRecordingRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        StartRecordingRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    StartRecordingRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    StartRecordingRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    StartRecordingRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    StartRecordingRequest &sbeRewind()
    {
    ...
    {
        StartRecordingRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    StartRecordingRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    StartRecordingRequest &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    StartRecordingRequest &streamId(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    StartRecordingRequest &sourceLocation(const SourceLocation::Value value) SBE_NOEXCEPT
    {
        std::int32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    StartRecordingRequest &putChannel(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    StartRecordingRequest &putChannel(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    StartRecordingRequest &putChannel(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const StartRecordingRequest &_writer)
{
    StartRecordingRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "StartRecordingRequest", )";
