...
#ifndef _AERON_ARCHIVE_CLIENT_PING_CXX_H_
#define _AERON_ARCHIVE_CLIENT_PING_CXX_H_
...
public:
    ...
    Ping(char *buffer, const std::uint64_t bufferLength) :
        Ping(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Ping(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Ping(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Ping &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Ping &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Ping skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Ping &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Ping &_writer)
{
    Ping writer(
        _writer.m_buffer,
        _writer.m_offset,
