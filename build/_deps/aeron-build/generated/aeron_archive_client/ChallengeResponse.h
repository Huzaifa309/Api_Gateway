...
#ifndef _AERON_ARCHIVE_CLIENT_CHALLENGERESPONSE_CXX_H_
#define _AERON_ARCHIVE_CLIENT_CHALLENGERESPONSE_CXX_H_
...
public:
    ...
    ChallengeResponse(char *buffer, const std::uint64_t bufferLength) :
        ChallengeResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ChallengeResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ChallengeResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ChallengeResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ChallengeResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ChallengeResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        ChallengeResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ChallengeResponse &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ChallengeResponse &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ChallengeResponse &putEncodedCredentials(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ChallengeResponse &putEncodedCredentials(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    ChallengeResponse &putEncodedCredentials(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ChallengeResponse &_writer)
{
    ChallengeResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ChallengeResponse", )";
...
SBE_NODISCARD static std::size_t computeLength(std::size_t encodedCredentialsLength = 0)
{
