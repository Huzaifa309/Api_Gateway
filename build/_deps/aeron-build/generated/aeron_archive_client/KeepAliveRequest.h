...
#ifndef _AERON_ARCHIVE_CLIENT_KEEPALIVEREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_KEEPALIVEREQUEST_CXX_H_
...
public:
    ...
    KeepAliveRequest(char *buffer, const std::uint64_t bufferLength) :
        KeepAliveRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    KeepAliveRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        KeepAliveRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    KeepAliveRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    KeepAliveRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    KeepAliveRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        KeepAliveRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    KeepAliveRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    KeepAliveRequest &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const KeepAliveRequest &_writer)
{
    KeepAliveRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "KeepAliveRequest", )";
