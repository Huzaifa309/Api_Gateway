...
#ifndef _AERON_ARCHIVE_CLIENT_RECORDINGPROGRESS_CXX_H_
#define _AERON_ARCHIVE_CLIENT_RECORDINGPROGRESS_CXX_H_
...
public:
    ...
    RecordingProgress(char *buffer, const std::uint64_t bufferLength) :
        RecordingProgress(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    RecordingProgress(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        RecordingProgress(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    RecordingProgress &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    RecordingProgress &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    RecordingProgress &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        RecordingProgress skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    RecordingProgress &recordingId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    RecordingProgress &startPosition(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    RecordingProgress &position(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const RecordingProgress &_writer)
{
    RecordingProgress writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "RecordingProgress", )";
