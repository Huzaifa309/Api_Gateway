...
#ifndef _AERON_ARCHIVE_CLIENT_FINDLASTMATCHINGRECORDINGREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_FINDLASTMATCHINGRECORDINGREQUEST_CXX_H_
...
class FindLastMatchingRecordingRequest
{
private:
...
public:
    ...
    FindLastMatchingRecordingRequest() = default;
    ...
    FindLastMatchingRecordingRequest(char *buffer, const std::uint64_t bufferLength) :
        FindLastMatchingRecordingRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    FindLastMatchingRecordingRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        FindLastMatchingRecordingRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    FindLastMatchingRecordingRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    FindLastMatchingRecordingRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    FindLastMatchingRecordingRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    FindLastMatchingRecordingRequest &sbeRewind()
    {
    ...
    {
        FindLastMatchingRecordingRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    FindLastMatchingRecordingRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    FindLastMatchingRecordingRequest &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD static const char *minRecordingIdMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t minRecordingIdId() SBE_NOEXCEPT
    {
        return 3;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t minRecordingIdSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool minRecordingIdInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t minRecordingIdEncodingOffset() SBE_NOEXCEPT
    {
        return 16;
    }

    static SBE_CONSTEXPR std::int64_t minRecordingIdNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t minRecordingIdMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t minRecordingIdMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t minRecordingIdEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t minRecordingId() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    FindLastMatchingRecordingRequest &minRecordingId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    FindLastMatchingRecordingRequest &sessionId(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    FindLastMatchingRecordingRequest &streamId(const std::int32_t value) SBE_NOEXCEPT
    {
        std::int32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    FindLastMatchingRecordingRequest &putChannel(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    FindLastMatchingRecordingRequest &putChannel(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    FindLastMatchingRecordingRequest &putChannel(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const FindLastMatchingRecordingRequest &_writer)
{
    FindLastMatchingRecordingRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "FindLastMatchingRecordingRequest", )";
    ...
    builder << ", ";
    builder << R"("minRecordingId": )";
    builder << +writer.minRecordingId();
