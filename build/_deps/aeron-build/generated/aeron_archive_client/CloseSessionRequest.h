...
#ifndef _AERON_ARCHIVE_CLIENT_CLOSESESSIONREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_CLOSESESSIONREQUEST_CXX_H_
...
public:
    ...
    CloseSessionRequest(char *buffer, const std::uint64_t bufferLength) :
        CloseSessionRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    CloseSessionRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        CloseSessionRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    CloseSessionRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    CloseSessionRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    CloseSessionRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        CloseSessionRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    CloseSessionRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const CloseSessionRequest &_writer)
{
    CloseSessionRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "CloseSessionRequest", )";
