...
#ifndef _AERON_ARCHIVE_CLIENT_RECORDINGDESCRIPTORHEADER_CXX_H_
#define _AERON_ARCHIVE_CLIENT_RECORDINGDESCRIPTORHEADER_CXX_H_
...
class RecordingDescriptorHeader
{
private:
...
public:
    ...
    RecordingDescriptorHeader() = default;
    ...
    RecordingDescriptorHeader(char *buffer, const std::uint64_t bufferLength) :
        RecordingDescriptorHeader(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    RecordingDescriptorHeader(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        RecordingDescriptorHeader(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    RecordingDescriptorHeader &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    RecordingDescriptorHeader &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    RecordingDescriptorHeader &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    RecordingDescriptorHeader &sbeRewind()
    {
    ...
    {
        RecordingDescriptorHeader skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    RecordingDescriptorHeader &length(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t stateEncodingLength() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD std::int32_t stateRaw() const SBE_NOEXCEPT
    {
        std::int32_t val;
    ...
    SBE_NODISCARD RecordingState::Value state() const
    {
        std::int32_t val;
        std::memcpy(&val, m_buffer + m_offset + 4, sizeof(std::int32_t));
        return RecordingState::get(SBE_LITTLE_ENDIAN_ENCODE_32(val));
    }

    RecordingDescriptorHeader &state(const RecordingState::Value value) SBE_NOEXCEPT
    {
        std::int32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    static SBE_CONSTEXPR std::int32_t checksumNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT32;
    }

    static SBE_CONSTEXPR std::int32_t checksumMinValue() SBE_NOEXCEPT
    {
        return INT32_C(-2147483647);
    }

    static SBE_CONSTEXPR std::int32_t checksumMaxValue() SBE_NOEXCEPT
    {
        return INT32_C(2147483647);
    }
    ...
    SBE_NODISCARD std::int32_t checksum() const SBE_NOEXCEPT
    {
        std::int32_t val;
    ...
    RecordingDescriptorHeader &checksum(const std::int32_t value) SBE_NOEXCEPT
    {
        std::int32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    RecordingDescriptorHeader &reserved(const std::int8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const RecordingDescriptorHeader &_writer)
{
    RecordingDescriptorHeader writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "RecordingDescriptorHeader", )";
