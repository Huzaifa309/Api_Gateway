...
#ifndef _AERON_ARCHIVE_CLIENT_STOPPOSITIONREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_STOPPOSITIONREQUEST_CXX_H_
...
public:
    ...
    StopPositionRequest(char *buffer, const std::uint64_t bufferLength) :
        StopPositionRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    StopPositionRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        StopPositionRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    StopPositionRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    StopPositionRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    StopPositionRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        StopPositionRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    StopPositionRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    StopPositionRequest &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    StopPositionRequest &recordingId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const StopPositionRequest &_writer)
{
    StopPositionRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "StopPositionRequest", )";
