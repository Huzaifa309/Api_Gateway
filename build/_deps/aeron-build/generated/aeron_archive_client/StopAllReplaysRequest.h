...
#ifndef _AERON_ARCHIVE_CLIENT_STOPALLREPLAYSREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_STOPALLREPLAYSREQUEST_CXX_H_
...
public:
    ...
    StopAllReplaysRequest(char *buffer, const std::uint64_t bufferLength) :
        StopAllReplaysRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    StopAllReplaysRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        StopAllReplaysRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    StopAllReplaysRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    StopAllReplaysRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    StopAllReplaysRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    StopAllReplaysRequest &sbeRewind()
    {
    ...
    {
        StopAllReplaysRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    StopAllReplaysRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    StopAllReplaysRequest &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    StopAllReplaysRequest &recordingId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const StopAllReplaysRequest &_writer)
{
    StopAllReplaysRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "StopAllReplaysRequest", )";
