...
#ifndef _AERON_ARCHIVE_CLIENT_STOPRECORDINGREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_STOPRECORDINGREQUEST_CXX_H_
...
public:
    ...
    StopRecordingRequest(char *buffer, const std::uint64_t bufferLength) :
        StopRecordingRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    StopRecordingRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        StopRecordingRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    StopRecordingRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    StopRecordingRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    StopRecordingRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    StopRecordingRequest &sbeRewind()
    {
    ...
    {
        StopRecordingRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    StopRecordingRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    StopRecordingRequest &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    StopRecordingRequest &streamId(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    StopRecordingRequest &putChannel(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    #if __cplusplus >= 201703L
    StopRecordingRequest &putChannel(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const StopRecordingRequest &_writer)
{
    StopRecordingRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "StopRecordingRequest", )";
