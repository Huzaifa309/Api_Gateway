...
#ifndef _AERON_ARCHIVE_CLIENT_REPLAYTOKENREQUEST_CXX_H_
#define _AERON_ARCHIVE_CLIENT_REPLAYTOKENREQUEST_CXX_H_
...
public:
    ...
    ReplayTokenRequest(char *buffer, const std::uint64_t bufferLength) :
        ReplayTokenRequest(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ReplayTokenRequest(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ReplayTokenRequest(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ReplayTokenRequest &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ReplayTokenRequest &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ReplayTokenRequest &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        ReplayTokenRequest skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ReplayTokenRequest &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ReplayTokenRequest &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ReplayTokenRequest &recordingId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ReplayTokenRequest &_writer)
{
    ReplayTokenRequest writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ReplayTokenRequest", )";
