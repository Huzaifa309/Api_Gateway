...
#ifndef _AERON_ARCHIVE_CLIENT_CONTROLRESPONSE_CXX_H_
#define _AERON_ARCHIVE_CLIENT_CONTROLRESPONSE_CXX_H_
...
public:
    ...
    ControlResponse(char *buffer, const std::uint64_t bufferLength) :
        ControlResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ControlResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ControlResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ControlResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ControlResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ControlResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ControlResponse &controlSessionId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ControlResponse &correlationId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD static const char *relevantIdMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t relevantIdId() SBE_NOEXCEPT
    {
        return 3;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t relevantIdSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool relevantIdInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t relevantIdEncodingOffset() SBE_NOEXCEPT
    {
        return 16;
    }

    static SBE_CONSTEXPR std::int64_t relevantIdNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t relevantIdMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t relevantIdMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t relevantIdEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t relevantId() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    ControlResponse &relevantId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD ControlResponseCode::Value code() const
    {
        std::int32_t val;
        std::memcpy(&val, m_buffer + m_offset + 24, sizeof(std::int32_t));
        return ControlResponseCode::get(SBE_LITTLE_ENDIAN_ENCODE_32(val));
    }

    ControlResponse &code(const ControlResponseCode::Value value) SBE_NOEXCEPT
    {
    ...
    ControlResponse &version(const std::int32_t value) SBE_NOEXCEPT
    {
        std::int32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    SBE_NODISCARD static const char *errorMessageMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static const char *errorMessageCharacterEncoding() SBE_NOEXCEPT
    {
        return "US-ASCII";
    }

    static SBE_CONSTEXPR std::uint64_t errorMessageSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool errorMessageInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    static SBE_CONSTEXPR std::uint16_t errorMessageId() SBE_NOEXCEPT
    {
        return 6;
    }

    static SBE_CONSTEXPR std::uint64_t errorMessageHeaderLength() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD std::uint32_t errorMessageLength() const
    {
        std::uint32_t length;
    ...
    SBE_NODISCARD const char *errorMessage()
    {
        std::uint32_t lengthFieldValue;
    ...
    std::uint64_t getErrorMessage(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ControlResponse &putErrorMessage(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    std::string getErrorMessageAsString()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    std::string getErrorMessageAsJsonEscapedString()
    {
        std::ostringstream oss;
        std::string s = getErrorMessageAsString();
    ...
    #if __cplusplus >= 201703L
    std::string_view getErrorMessageAsStringView()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    ControlResponse &putErrorMessage(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putErrorMessage(str.data(), static_cast<std::uint32_t>(str.length()));
    }

    #if __cplusplus >= 201703L
    ControlResponse &putErrorMessage(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ControlResponse &_writer)
{
    ControlResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ControlResponse", )";
    ...
    builder << ", ";
    builder << R"("relevantId": )";
    builder << +writer.relevantId();
    ...
    builder << ", ";
    builder << R"("errorMessage": )";
    builder << '"' <<
        writer.getErrorMessageAsJsonEscapedString().c_str() << '"';
...
SBE_NODISCARD static std::size_t computeLength(std::size_t errorMessageLength = 0)
{
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += errorMessageHeaderLength();
    if (errorMessageLength > 1073741824LL)
    {
        throw std::runtime_error("errorMessageLength too long for length type [E109]");
    }
