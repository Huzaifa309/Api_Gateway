...
void responseDispatcherThread() {
    ...
    while (true) {
        if (ResponseQueue.try_dequeue(task)) {
            ...
            try {
                // Check if this is an Aeron response (no callback) or HTTP response (with callback)
       ...
       Logger::getInstance().log("[T4] Dequeued callback task for dispatch");
        ...
        Logger::getInstance().log("[T4] No callback task dequeued");
                    ...
                    Logger::getInstance().log("[T4] Aeron SBE response received: " + task.json);
                    
                    // TODO: Here you could implement additional processing for Aeron responses
                    // such as storing in a database, forwarding to other services, etc.
             
            } catch (const std::exception &e) {
                Logger::getInstance().log(std::string("[T4] Error processing response: ") + e.what());
